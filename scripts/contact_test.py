...
class ur5e_admittance():
    ...
    wrench = np.zeros(6)
    contact_flag = np.zeros(2)
    previous_contact_flag = np.zeros(2)
    ...
    def __init__(self, test_control_signal = False, conservative_joint_lims = True):
        ...
        rospy.Subscriber("rdda_master_output", RDDAPacket, self.contact_callback)
    ...
    def move(self,
        ...
        while not self.shutdown and self.safety_mode == 1: #chutdown is set on ctrl-c.
            ...
            if (self.contact_flag[1] > self.previous_contact_flag[1]):
                print('contact happen!')
                pos_index = (pos_index + 1) % 2
            self.previous_contact_flag = self.contact_flag
